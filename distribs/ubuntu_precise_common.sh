...
ubuntu_precise_media_info_install()
{
	export TEXTDOMAINDIR=$CURRENT/locale
	export TEXTDOMAIN=mediaspip
...
ubuntu_precise_dep_install()
{
	...
	apt-get -q -y --force-yes install build-essential subversion git-core checkinstall libcxxtools-dev scons libboost-dev zlib1g-dev unzip \
		apache2 mysql-server php5-dev php-pear php5-curl php5-gd php5-imagick libapache2-mod-php5 re2c texi2html \
	...
	ubuntu_precise_media_info_install || return 1
