...
# debian_stretch
# 
# Installation des dépendances pour Debian Stretch
#
...
# Pour Ubuntu quantal
debian_stretch_dep_install()
{
	export TEXTDOMAINDIR=$CURRENT/locale
	export TEXTDOMAIN=mediaspip
	
	DEBIANMULTIMEDIA=$(grep "deb-multimedia" /etc/apt/sources.list |grep jessie) 2>> $LOG >> $LOG
	...
	#		echo "deb http://www.deb-multimedia.org jessie main non-free" >> /etc/apt/sources.list 2>> $LOG
	...
	apt-get -q -y --force-yes install build-essential curl subversion git-core checkinstall libcxxtools-dev yasm scons libboost-dev zlib1g-dev unzip \
		apache2 mysql-server php-dev php-pear php5-mysql php-sqlite3 php5-curl php5-gd php-imagick libapache2-mod-php5 re2c \
		libmp3lame-dev libfaac-dev libfaad-dev libmodplug-dev libgsm1-dev libopenjp2-7-dev libxvidcore-dev libtheora-dev libspeex-dev libopencore-amrnb-dev libopencore-amrwb-dev libvorbis-dev libass-dev libtwolame-dev libopus-dev librtmp-dev\
		flac vorbis-tools imagemagick pngnq optipng libjpeg-progs unrar-free \
...
debian_stretch_ffmpeg_install ()
{
	...
	if [ "$FFMPEG_VERSION" = "$VERSION_ACTUELLE" ] && [ "$FFMPEG_FORCE_INSTALL" = "non" ];then
		...
		./configure --disable-doc --disable-ffplay --disable-ffserver --enable-gpl --enable-version3 --enable-nonfree --enable-shared --enable-postproc --enable-pthreads --enable-libvpx \
			...
			--enable-libopus --enable-libmodplug --enable-libspeex --enable-libopenjpeg --enable-libgsm --enable-avfilter --enable-zlib --enable-libass --enable-libtwolame 2>> $LOG >> $LOG
		echo $(eval_gettext "Info compilation make")
		make -j $NO_OF_CPUCORES 2>> $LOG >> $LOG || return 1
