...
debian_lame_install()
{
	...
	if [ ! -e "$SRC_INSTALL"/lame-3.98.4.tar.gz ]; then
		...
		eval_gettext "Info debut lame install" 2>> $LOG  >> $LOG
...
debian_media_info_install()
{
	...
	if [ ! -e "$SRC_INSTALL"/MediaInfo_CLI_0.7.38_GNU_FromSource.tar.bz2 ];then
		echo
		eval_gettext "Info debut mediainfo install"
		eval_gettext "Info debut mediainfo install" 2>> $LOG >> $LOG
		cd $SRC_INSTALL
		...
		eval_gettext "End mediainfo"
	elif [ "$MEDIAINFOVERSION" == "v0.7.38" ]; then
		echo
		eval_gettext "Info a jour mediainfo"
		eval_gettext "Info a jour mediainfo" 2>> $LOG >> $LOG
	else
		eval_gettext "Info debut mediainfo update"
		eval_gettext "Info debut mediainfo update" 2>> $LOG >> $LOG
...
debian_phpimagick_install()
{
	...
	if [ "$ACTUEL" == "-" ]; then
		# Installation
		eval_gettext "Info debut php-imagick install"
		eval_gettext "Info debut php-imagick install" 2>> $LOG >> $LOG
		echo autodetect | pecl install imagick 2>> $LOG >> $LOG
		eval_gettext "End php-imagick"
		eval_gettext "End php-imagick" 2>> $LOG >> $LOG
	# Cas où on a déjà installé la dernière version
	elif [ "$ACTUEL" == "$LATEST" ]; then
		# Déjà à jour
		eval_gettext "Info a jour php-imagick"
		eval_gettext "Info a jour php-imagick" 2>> $LOG >> $LOG
	# Cas de la mise à jour
	else
		# Mise à jour
		eval_gettext "Info debut php-imagick update"
		eval_gettext "Info debut php-imagick update" 2>> $LOG >> $LOG
...
debian_x264_update ()
{
	...
	if [ "$REVISION" == "$NEWREVISION" ]; then
		echo
		eval_gettext "Info a jour x264"
		eval_gettext "Info a jour x264" 2>> $LOG  >> $LOG
	else
...
debian_ffmpeg_update ()
{
	...
	if [ -x /usr/local/bin/ffmpeg ];then
		...
		if [ "$VERSION" = "$REVISION_VERSION" ];then
			eval_gettext "Info a jour ffmpeg"
			eval_gettext "Info a jour ffmpeg" 2>> $LOG >> $LOG
		else
...
debian_ffmpeg_php_update ()
{
	...
	if [ "$OLDREVISION" = "$REVISION" ];then
		eval_gettext "Info a jour ffmpeg-php"
		eval_gettext "Info a jour ffmpeg-php" 2>> $LOG >> $LOG
	else
		phpize
		make -j $NO_OF_CPUCORES clean 2>> $LOG >> $LOG
		make -j $NO_OF_CPUCORES distclean 2>> $LOG >> $LOG
